...
function datashReadDate(val, includeTime) {
 ...
 */
function datashAddCommonValidationRules(objectType, id) {
    Validation.addAllThese([
        ['validate-nospace', Zikula.__('No spaces', 'module_Datasheete'), function(val, elem) {
            var valStr;
            valStr = new String(val);
            return (valStr.indexOf(' ') === -1);
        }],
        ['validate-htmlcolour', Zikula.__('Please select a valid html colour code.', 'module_Datasheete'), function(val, elem) {
            var valStr;
            valStr = new String(val);
            return Validation.get('IsEmpty').test(val) || (/^#[0-9a-f]{3}([0-9a-f]{3})?$/i.test(valStr));
        }],
        ['validate-upload', Zikula.__('Please select a valid file extension.', 'module_Datasheete'), function(val, elem) {
