...
var datashContextMenu;

datashContextMenu = Class.create(Zikula.UI.ContextMenu, {
 ...
 */
function datashInitItemActions(objectType, func, containerId) {
    var triggerId, contextMenu, iconFile;

    triggerId = containerId + 'trigger';

    // attach context menu
    contextMenu = new datashContextMenu(triggerId, { leftClick: true, animation: false });
...
function datashCapitaliseFirstLetter(string) {
 ...
 */
function datashSubmitQuickNavForm(objectType) {
    $('datash' + datashCapitaliseFirstLetter(objectType) + 'QuickNavForm').submit();
}

/**
 * Initialise the quick navigation panel in list views.
 */
function datashInitQuickNavigation(objectType, controller) {
    if ($('datash' + datashCapitaliseFirstLetter(objectType) + 'QuickNavForm') == undefined) {
        return;
    }

    if ($('catid') != undefined) {
        $('catid').observe('change', function () { datashSubmitQuickNavForm(objectType); });
    }
    if ($('sortby') != undefined) {
        $('sortby').observe('change', function () { datashSubmitQuickNavForm(objectType); });
    }
    if ($('sortdir') != undefined) {
        $('sortdir').observe('change', function () { datashSubmitQuickNavForm(objectType); });
    }
    if ($('num') != undefined) {
        $('num').observe('change', function () { datashSubmitQuickNavForm(objectType); });
    }

    switch (objectType) {
    case 'datasheet':
        if ($('workflowState') != undefined) {
            $('workflowState').observe('change', function () { datashSubmitQuickNavForm(objectType); });
        }
        if ($('kind') != undefined) {
            $('kind').observe('change', function () { datashSubmitQuickNavForm(objectType); });
        }
        break;
    default:
        break;
