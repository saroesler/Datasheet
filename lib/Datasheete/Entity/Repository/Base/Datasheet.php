 ...
 *
 * This is the base repository class for datasheet entities.
 */
class Datasheete_Entity_Repository_Base_Datasheet extends EntityRepository
{
    /**
    ...
    public function getDescriptionFieldName()
   {
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $listHelper = new Datasheete_Util_ListEntries($serviceManager);
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('datasheet', 'workflowState');
                $templateParameters['kindItems'] = $listHelper->getEntries('datasheet', 'kind');
            }
    
            // initialise Imagine preset manager instances
            $imageHelper = new Datasheete_Util_Image(ServiceUtil::getManager());
    
            $objectType = 'datasheet';
            $parameters[$objectType . 'ThumbManagerDatasheet'] = $imageHelper->getManager($objectType, 'datasheet', $context, $args);
            if (in_array($args['action'], array('display', 'view'))) {
                // use seperate preset for images in related items
                $parameters['relationThumbPreset'] = $imageHelper->getPreset('', '', 'Datasheete_relateditem', $context, $args);
            }
        }
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters['kind'] = FormUtil::getPassedValue('kind', '', 'GET');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('Datasheete_Entity_Datasheet', 'tbl');
        $query = $qb->getQuery();
        $query->execute();
    }
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('Datasheete_Entity_Datasheet', 'tbl')
     ...
     *
     * @return array|Datasheete_Entity_Datasheet retrieved data array or Datasheete_Entity_Datasheet instance
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved Datasheete_Entity_Datasheet instances
     */
    ...
    protected function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $categoryFiltersPerRegistry = ModUtil::apiFunc('Datasheete', 'category', 'buildFilterClauses', array('ot' => 'datasheet', 'catids' => $v));
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters)
    {
        ...
        if ($currentType == 'admin' && $currentModule == 'Datasheete') {
            return $qb;
        }
    
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved datasheets only
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.kind = \'' . $fragment . '\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
        $where .= 'tbl.datasheet = \'' . $fragment . '\'';
        $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numDatasheets';
        if ($useJoins === true) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('Datasheete_Entity_Datasheet', 'tbl');
     ...
     * @param int    $excludeId  Id of datasheets to exclude (optional).
     *
    ...
    protected function getQueryFromBuilder(QueryBuilder $qb)
    {
        ...
        //$fu = new FilterUtil('Datasheete', $this);
